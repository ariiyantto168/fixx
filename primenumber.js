...
function numberPrimeCruncher(input) {
    ...
    ///jika input kurang dari sama dengan 1
    if (input <= 1) {
        return hasil
    }

    ///cari kumpulan angka prima
    ...
    ///hitung rerata kumpulan angka prima
    for (var j = 0; j < prima.length; j++) {
        rerata += prima[j]
    }
    rerata /= prima.length
    
    ///masukkin ke hasil jika angka prima lebih dari rerata
    for (var h = 0; h < prima.length; h++) {
        if (prima[h] > rerata) {
            hasil.push(prima[h])
        }
    }
  ...
  console.log(numberPrimeCruncher(44)); // [ 25, 29, 31, 37, 41, 43 ]
  console.log(numberPrimeCruncher(23)); // [ 17, 19, 23 ]
  console.log(numberPrimeCruncher(1)); // []
  console.log(numberPrimeCruncher(0)); // []

  ///waktu pengerjaan 8 menit 55 detik