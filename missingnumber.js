...
function missingNum ( arr ) {
    ...
    if (arr.length == 0) {
        ...
        for (var i = 0; i < arr.length; i++) {
            for (var j = 0; j < arr[i].length; j++) {
                if (typeof arr[i][j] === 'number') {
                    angkaAda.push(arr[i][j])
                }
            }
        }

        for (var a = 1; a < angkaAda.length; a++) {
            for (var b = 0; b < a; b++) {
                if (angkaAda[a] < angkaAda[b]) {
                    var tukar = angkaAda[a]
                    angkaAda[a] = angkaAda[b]
                    angkaAda[b] = tukar
                }
            }
        }
        ...
        terkecil = angkaAda[0]
        terbesar = angkaAda[angkaAda.length-1]

        for (var h = terkecil; h < terbesar; h++) {
            var cek = true
            for (k = 0; k < angkaAda.length; k++) {
                if (h == angkaAda[k]) {
                    cek = false
                }
            }
